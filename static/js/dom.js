...
export function renderProtesterCount(count) {
  const el = document.getElementById('number-of-protesters');
  el.innerHTML = `${pre(count)} ${count} ${title(count)}`;
}

export function renderProtesters({ protesters, joined, myId }) {
  const protesterContainerEl = document.getElementById('protesters');
  protesterContainerEl.innerHTML = '';
  ...
  protesters.forEach(protester => {
    ...
    protesterContainerEl.appendChild(protesterEl);
  });
}

function generateProtesterEl(id) {
...
export function renderProtesting({ id }) {
  const protesterEl = document.getElementById(id);
  if (!protesterEl) return;

  const activeProtesterSvg = protesterEl.getElementsByTagName('svg')[0];
  if (activeProtesterSvg) {
    activeProtesterSvg.classList.add('shake-vertical');
    setTimeout(() => {
      activeProtesterSvg.classList.remove('shake-vertical');
    }, PROTEST_TIME);
  }
}

export function renderTopics({ topicsWithCounts }) {
  ...
  nonEmptyTopics.forEach(topicWithCount => {
    ...
    topicLink.innerText = `${topicWithCount.topicName.toUpperCase()} (${topicWithCount.protesterCount})`;
